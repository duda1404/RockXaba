...
if (!empty($chave)){
    ...
    $verifica_chave = mysqli_query($connect, "SELECT id_user FROM usuario WHERE chave = '$chave' LIMIT 1");
    ...
    if (mysqli_num_rows($verifica_chave)>0){
        ...
        $update = "UPDATE usuario SET sit_user_id = 1  WHERE id_user = '".$array['id_user']."'";
        $confirma_conta = mysqli_query($connect, $update);

        $deleta_chave =  "UPDATE usuario SET chave = NULL WHERE id_user = '".$array['id_user']."'";
