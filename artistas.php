...
no banco. Enquanto houver resultado da consulta no MYSQL, executará o loop que preenche as informações*/
			$image_query = pg_query($connect, "select artista.id_artista, artista.nome_artista, 
				...
				foto_artista on artista.id_artista = foto_artista.FK_ARTISTA_id_artista where fk_situacao_id_sit = 1 order by artista.dat_add_artista desc");
			...
			?>
				...
				<li>
					<a href="artista_page.php?myid=<?php echo $id_artista; ?>" target="new window">
						<img src="uploads/<?php echo $photo; ?>" class="testando" alt="" title="<?php echo $nome_artista; ?>" name="<?php echo $nome_artista; ?>" />
					</a>
				</li>
			<?php
			}
